...
function toModelAttribution(input, fallbackTimestamp) {
...
function toNowcastChart(nowcast, fallbackTimestamp) {
...
export function toMacroSnapshot(raw) {
    ...
    const headlineMetrics = (raw.headline ?? []).map((metric, index) => {
        ...
        const deltaPct = deltaAbs === null || baselineValue === null || baselineValue === 0
            ? null
    ...
    return {
        ...
        caveats: (raw.caveats ?? []).map((caveat, index) => ({
            ...
            severity: caveat.severity === 'info' || caveat.severity === 'warning' || caveat.severity === 'critical'
                ? caveat.severity
                : 'warning',
