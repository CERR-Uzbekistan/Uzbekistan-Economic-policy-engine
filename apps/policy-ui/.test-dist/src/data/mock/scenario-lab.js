...
export function getDefaultAssumptionState() {
    return SCENARIO_ASSUMPTIONS.reduce((acc, assumption) => {
        acc[assumption.key] = assumption.default_value;
        return acc;
    }, {});
}
export function applyPresetToState(presetId) {
...
function getMetricCore(values) {
    ...
    const gdpGrowth = clamp(roundTo(5.8 +
        0.16 * govSpending -
        0.12 * policyRate -
        0.05 * fx +
        0.04 * externalDemand +
        0.03 * remittance -
        0.03 * riskPremium), 2.5, 8.5);
    const inflation = clamp(roundTo(8.6 +
        0.24 * fx +
        0.11 * commodity +
        0.2 * passThrough -
        0.16 * policyRate -
        0.04 * tariff +
        0.03 * govSpending), 4.5, 16.5);
    const currentAccount = clamp(roundTo(-2.2 +
        0.09 * fx +
        0.08 * externalDemand +
        0.06 * tariff -
        0.06 * govSpending -
        0.04 * remittance -
        0.03 * commodity), -6, 2.5);
...
function buildHeadlineMetrics(values) {
...
function buildSeriesPath(baseValue, scenarioValue, softness = 0.35) {
...
function buildChartSeries(title, subtitle, axisLabel, unit, baselineValue, scenarioValue, takeaway) {
    return {
...
function buildInterpretation(values) {
    ...
    const majorDrivers = Object.entries(values)
        ...
        .slice(0, 2)
        .map(([key]) => key);
    const driverSummary = majorDrivers.length > 0
...
export function buildScenarioLabResults(values) {
    ...
    return {
        ...
        charts_by_tab: {
            ...
            macro_path: buildChartSeries('Macro path (real GDP growth)', 'Baseline and scenario trajectories', 'GDP growth', '%', baselineCore.gdpGrowth, scenarioCore.gdpGrowth, 'Growth path reflects combined demand, cost, and policy-rate channels.'),
            external_balance: buildChartSeries('External balance path (current account)', 'Baseline and scenario trajectories', 'Current account', '% GDP', baselineCore.currentAccount, scenarioCore.currentAccount, 'External balance responds to exchange-rate, trade, and remittance assumptions.'),
            fiscal_effects: buildChartSeries('Fiscal effects path (fiscal balance)', 'Baseline and scenario trajectories', 'Fiscal balance', '% GDP', baselineCore.fiscalBalance, scenarioCore.fiscalBalance, 'Fiscal outcomes are driven by spending and revenue assumptions in this mock setup.'),
        },
        interpretation,
    };
}
export const scenarioLabWorkspaceMock = {
