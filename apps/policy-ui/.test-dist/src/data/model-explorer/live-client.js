...
export function resolveModelExplorerApiUrl() {
...
export function resolveModelExplorerTimeoutMs() {
    const metaEnv = readImportMetaModelExplorerEnv();
    const processEnv = readProcessModelExplorerEnv();
...
export async function fetchModelExplorerLiveRawPayload(fetchImpl = fetch) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new ModelExplorerTransportError('http', `Model Explorer request failed with HTTP ${response.status}.`, { status: response.status });
        }
        ...
        if (isAbortError(error)) {
            throw new ModelExplorerTransportError('timeout', `Model Explorer request timed out after ${timeoutMs}ms.`, { cause: error });
        }
